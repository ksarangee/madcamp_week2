...
class _EditDocumentScreenState extends State<EditDocumentScreen> {
  ...
  void initState() {
    ...
    _imageUrlController = TextEditingController(text: widget.document.imageUrl ?? '');
    _selectedCategory = _categories[widget.document.categoryId - 1]; // 기존 카테고리를 초기값으로 설정
  }
  ...
  try {
    ...
    var request = http.MultipartRequest('PUT', Uri.parse('$backendUrl/edit_post/${widget.document.id}'));
    ...
    request.fields['today_views'] = (widget.document.todayViews ?? 0).toString();
    ...
    if (response.statusCode == 200) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to update document. Please try again.')),
      );
    }
  } catch (e) {
    setState(() {
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: _selectedCategory != null && !_isLoading ? _saveDocument : null,
          ),
        ],
      ),
      body: SafeArea(
